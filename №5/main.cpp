...
void getTime();									//Получить время RTC
void setTime();									//Установить время RTC
void setDelay();								//Задержка
void setAlarm();								//Установить будильник
void resetAlarm();								
void inputTime();								//Ввод
char getBCD(int);
...
void interrupt newTimer(...){
	...
	delayCounter--;								//Уменьшить счетчик
...
int main(){
	...
	oldAlarm = getvect(0x4A);						//Получить старый обработчик прерывания будильника
...
void getTime() {
	...
	int date[6];
	char month[][10] = { "January", "February", "March", "April", "May", "June", "July", "August",
